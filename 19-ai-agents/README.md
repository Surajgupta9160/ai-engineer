...
**Builds on:** Module 18 (Tools & Function Calling) — agents USE tools. The tool-calling mechanics (JSON schemas, function execution, returning results) are the building blocks that agents orchestrate in a loop. Module 09 (LLM internals) explains why the LLM can reason about tool outputs. Module 14 (Prompt Engineering) — the ReAct pattern covered there is the core agent loop used here.

**New concepts introduced here:** Agent architectures (ReAct, Plan-and-Execute), memory types, multi-agent systems, guardrails against runaway loops, and production safety for autonomous actions.
