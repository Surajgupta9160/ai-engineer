...
**Builds on:** Module 08 (Word2Vec, GloVe introduced the concept of dense word vectors). This module extends that to modern sentence and document embeddings. Also uses Module 05 concepts (neural network training, loss functions) for the advanced training section at the end.

**Used by:** Module 16 (Vector Databases store embedding vectors), Module 17 (RAG retrieves by embedding similarity), Module 22 (fine-tuning trains embedding models).
...
## Advanced: How Embedding Models Are Trained

### Contrastive Learning and Losses

```
CONTRASTIVE LEARNING — CORE IDEA:
  Learn embeddings where similar pairs cluster together, dissimilar pairs apart.
  No fixed labels needed — similarity is defined by the training pair structure.

INFONCE LOSS (used by CLIP, E5, text-embedding-3 family):
  For a batch of N (anchor, positive) pairs, treat all other positives
  in the batch as negatives for each anchor.

  L = -log( exp(sim(a, p) / τ) / Σ_j exp(sim(a, x_j) / τ) )
  where τ = temperature (typically 0.05-0.1), sum is over all other examples

  Effect: pushes (a, p) together; pushes (a, everything else) apart
  Larger batches = more negatives = stronger training signal
  Google/OpenAI use batch sizes of 16K-65K

MNRL LOSS (Multiple Negatives Ranking Loss, sentence-transformers standard):
  Same structure as InfoNCE without explicit temperature.
  One forward pass per batch creates N² training signals (very efficient).
  Used by: all-MiniLM, all-mpnet, BGE, E5 base variants

HARD NEGATIVE MINING (critical for quality):
  Random negatives are too easy — the model learns nothing from them.
  Hard negatives: semantically similar but NOT the correct match.
  Example: "When was Python created?" + hard_neg = "When was Java created?"

  Hard negative sources:
    BM25 hard negatives: top BM25 results that aren't the gold answer
    Cross-encoder hard negatives: passages that a cross-encoder ranks high
      but that are NOT the gold passage (model-mined)
  Training with hard negatives is the #1 quality lever for embedding models.
```

### Matryoshka Representation Learning (MRL)

```
PROBLEM: Fixed 1536-dim embeddings force a tradeoff:
  More dims = better quality = more storage/compute
  Fewer dims = worse quality = cheaper
  No flexibility to tune per use case.

MATRYOSHKA EMBEDDINGS (Kusupati et al. 2022; adopted by OpenAI 2024):
  Train so that ANY prefix of the embedding vector is itself a valid embedding.

  Training: add contrastive loss at multiple prefix lengths simultaneously:
    L = Σ_m λ_m × L_contrastive(embeddings[:m])
    m ∈ {64, 128, 256, 512, 1024, 1536}

  At inference: truncate to any supported dimension — no quality cliff.

  OpenAI API:
    response = client.embeddings.create(
        model="text-embedding-3-small",
        input="...",
        dimensions=256  # Returns 256-dim matryoshka embedding
    )

  Quality vs dimension (text-embedding-3-small, MTEB benchmark):
    1536 dims: 62.3% (baseline)
    512 dims:  61.6% (3× storage savings, −0.7% quality)
    256 dims:  60.1% (6× savings, −2.2% quality)
  → 512 dims is the production sweet spot for most RAG systems.
```

### Bi-Encoder vs Cross-Encoder — Retrieval Architecture

```
BI-ENCODER (for ANN/vector search):
  Encodes query and document INDEPENDENTLY.
  Similarity = cosine(embed(query), embed(doc))
  All document embeddings pre-computed and stored in vector index.
  Query time: O(1) — embed query, ANN search.
  Quality: misses fine-grained query-document token interactions.

CROSS-ENCODER (for reranking):
  Concatenates [query; document] and encodes them TOGETHER.
  Full cross-attention between all query and document tokens.
  Quality: much higher — sees exactly which query terms match which document terms.
  Speed: O(K) at query time — must run inference per (query, doc) pair.
  Cannot pre-compute → only viable for top-K reranking (K ≤ 50).

PRODUCTION RETRIEVAL PIPELINE:
  Stage 1 — Bi-encoder retrieval: embed query → ANN search → top 100 candidates
  Stage 2 — Cross-encoder reranking: score each candidate → return top 10
  Stage 3 — LLM generation using top 10 candidates as context

  This two-stage approach combines the speed of ANN with the quality of
  full cross-attention. Quality improvement: typically 5-15% recall@10.

ASYMMETRIC TRAINING (query ≠ document):
  Short queries (5-10 tokens) vs long passages (100-500 tokens).
  Modern models use instruction prefixes to handle this:
...
---
*Next: [16 — Vector Databases](../16-vector-databases/README.md)*
