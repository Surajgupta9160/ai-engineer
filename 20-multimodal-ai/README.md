...
## VLM Architecture Deep Dive — Principal-Level
...
```
PIPELINE:
  1. Vision Encoder: CLIP ViT-L/14 processes the image
     - CLIP was trained with contrastive learning on (image, text) pairs
     - ViT-L/14: Large ViT with 14×14 pixel patch size
     - Output: 256 visual tokens (for a 224×224 image, 16×16 patches)
       each token is a 1024-dim embedding

  2. Projection: a 2-layer MLP maps visual tokens to the LLM's
     embedding dimension
     visual_tokens (256, 1024) → MLP → visual_tokens (256, 4096)
     This "teaches" the LLM to understand CLIP's feature space.

  3. Concatenation: visual tokens prepended to text tokens
     final_sequence = [visual_tok_1, ..., visual_tok_256, text_tok_1, ...]
     The LLM processes this as a single sequence.
     Attention is bidirectional over the full sequence (image + text).

  4. Autoregressive generation: LLM generates text normally.

TRAINING:
  Stage 1: Freeze LLM and vision encoder, train only the MLP projection
           on image-caption pairs → alignment pre-training
  Stage 2: Unfreeze all, fine-tune on instruction-following visual QA data

CLIP as vision backbone:
  CLIP (Radford et al., OpenAI 2021) learned visual representations
  that are semantically aligned with language by training on
  400M (image, text) pairs with contrastive loss.
  The representations naturally correspond to text descriptions
  → ideal for connecting to an LLM.
```

---

### Architecture Approach 2: Cross-Attention Fusion (Flamingo-style)

Used in Flamingo (DeepMind), Idefics, and variants.

```
PIPELINE:
  1. Frozen vision encoder (NFNet ResNet or ViT) processes images
  2. Perceiver Resampler: reduces variable number of image patches
     to a fixed 64 "visual tokens" (learned compression)
  3. New cross-attention layers interleaved into the frozen LLM:
     At every N LLM layers, insert a cross-attention block:
       - LLM hidden states attend over visual token keys/values
       - Standard transformer cross-attention
       - LLM weights frozen; only cross-attention layers trained

ADVANTAGES:
  - LLM weights are preserved (no interference with text capability)
  - Can handle multiple images naturally (each image adds cross-attn keys)
  - The Perceiver Resampler creates a constant-size visual representation
    regardless of image resolution

DISADVANTAGES:
  - Cross-attention adds latency at every inserted layer
  - Complex to implement and train
  - Visual information injected indirectly (through new weights)
    vs directly (through token sequence in LLaVA)

TOKEN PROJECTION vs CROSS-ATTENTION:
...
### Dynamic Tiling — Handling Fine-Grained Visual Tasks

Standard ViT patch tokenization fails on tasks requiring fine-grained spatial precision: OCR on dense documents, circuit diagrams, medical images, data tables.

```
THE PROBLEM:
  ViT-L/14: 14×14 pixels per patch, 224×224 input = 256 patches
  Effective resolution per patch: 14×14 pixels
  For a 1-point font text in a document: text may span only 5-10 pixels
  → Sub-patch features are invisible to the model

  At 16×16 patches (standard ViT), a 200×200 pixel table with 20 rows
  might have entire rows within a single patch — completely unresolvable.

DYNAMIC TILING (LLaVA-1.6 / LLaVA-HD, 2024):
  Instead of resizing to 224×224, preserve resolution by splitting:

  Algorithm:
  1. Determine best tiling from a set of candidate tile grids:
     1×2, 2×1, 2×2, 1×3, 3×1, 2×3, etc.
  2. Choose the grid that minimizes aspect ratio distortion
     while fitting within a total tile budget (e.g., ≤12 tiles)
  3. Split the image into tiles; resize each tile to 336×336
  4. Process each tile independently through the vision encoder
  5. ALSO process a low-resolution thumbnail of the full image
     (provides global context — where each tile fits in the whole)
  6. Concatenate all tile tokens + thumbnail tokens → full representation

  Example for a wide document image (1200×400 pixels):
    Best grid: 3×1 (3 horizontal tiles, 1 row)
    Each tile: 400×400 pixels (padded)
    Processed at 336×336 via ViT
    Tokens: 3 × 576 tile tokens + 576 thumbnail tokens = 2304 tokens

  Result:
    Effective resolution: 1008×336 (3 tiles × 336)
    vs standard: 336×336 (single resize)
    Fine-grained text, tables, charts become resolvable.

TOKEN COUNT TRADEOFF:
  Standard (1 image): 576 tokens
  4-tile dynamic: ~2304 tokens (4× more)
  Higher-resolution → more tokens → slower inference, more cost
  Must balance resolution with context window limits and latency budget

MODELS USING DYNAMIC TILING:
  LLaVA-1.6 (haiku): up to 4 tiles
  LLaVA-HD: up to 9 tiles at 672×672 per tile
  InternVL2: dynamic high-resolution with up to 40 tiles
  GPT-4V: internal tiling (undisclosed), 2048×2048 max resolution
  Claude 3: tiling with 1568 tokens per image (Anthropic's approach)

WHEN SPATIAL PRECISION IS CRITICAL:
  OCR on scanned documents → use high-res tiling
  Reading lab reports, prescriptions → high-res tiling required
  Data table extraction → medium tiling (2×2 minimum)
  General image captioning → standard 1×1 tile sufficient
  Logo identification → standard sufficient
```

---

### Spatial Reasoning vs Captioning — Architectural Tradeoffs

```
TASK REQUIREMENTS:

High-level captioning ("describe this image"):
  - Needs: global semantic understanding
  - Tokens needed: 256-576 (standard resolution)
  - CLIP features work well — trained on image-level labels
  - Temperature: can be higher (0.3-0.7)
  - Example models: GPT-4o (fast mode), Claude Haiku

Fine-grained spatial reasoning ("what does cell B3 say in this table"):
  - Needs: precise local feature at specific coordinates
  - Tokens needed: 1000-4000 (tiled high-resolution)
  - CLIP features insufficient alone — CLIP pools global context
  - Need: either dynamic tiling OR dedicated OCR+layout model
  - Temperature: 0 (exact extraction)
  - Example models: GPT-4o (detailed mode), Claude 3 Opus

Circuit diagram interpretation:
  - Needs: component symbols, wire connections, label text
  - Tokens needed: 2000+ (very high resolution)
  - Text labels on components: OCR sub-task
  - Topology: spatial relationship understanding
  - Best approach: high-res tiling + chain-of-thought
  - Alternative: hybrid — use OCR first, describe to LLM as text

PRODUCTION HEURISTIC:
  If the user task requires reading text that appears in the image
  (receipts, documents, forms, code screenshots):
    → Enable high-resolution mode / maximum tiling
    → Use OCR-specialized model where available

  If the user task requires only understanding visual content
  (scene description, object recognition, face identification):
    → Standard resolution is sufficient and cheaper
```
